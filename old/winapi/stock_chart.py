...
def get_min_period_data(code, startdate, enddate):
    ...
    _, result[len(result):len(result)] = get_period_data_raw(code, pivot, enddate)
    return result
...
if __name__ == "__main__":
    n = datetime.now()
    _, d = get_period_data_raw('A005930', datetime.now() - timedelta(days=365), datetime(n.year, n.month, n.day) - timedelta(days=1), period_type='D')
    count = 0
    for i in d:
        print(i)
        count += 1
