...
class PriceTrendLine:
    ...
    def __init__(self, data):
        ...
        def inc():
            self.i += 1
            return self.i
        self.price_list = list(map(lambda x: {'date': time_converter.intdate_to_datetime(x['0']), 'close': x['5'], 'xaxis': inc(), 'volume': x['6']}, data))
        self.df = pd.DataFrame(self.price_list)
        self.df = self.df.set_index('date')

    def get_date(self):
        return self.df.date[-1]
        ...
        for i, v1 in enumerate(reversed(self.price_list)):
            ...
            for v2 in reversed(self.price_list[:left_jump_index]):
                ...
                if found:
                    ...
                    return [v1['date'], v2['date']], [v1['close'], v2['close']], ((y2 - y1) / (x2 - x1)) > 0
    ...
    for code in code_list:
        ...
        while datetime.now() > today:
            ...
            if l == 0: 
                ...
                _, data = stock_chart.get_day_period_data(code, today - timedelta(days=90), today)
                ...
                df = df.append({'code': code, 'date': today, 'price': ptl.get_close_price(),
        ...
        print({'code': code, 'profit': total_profit})
