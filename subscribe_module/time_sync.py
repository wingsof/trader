...
def do_sync():
    ...
    print( 'Got time as', dt_.strftime( '%Y-%m-%d %H:%M:%S'), 'from NTP Server' , flush=True)
    ...
    if ret == 0:
        print( 'Setting failed. Try as administrator.', flush=True)
    else:
        print( 'Successfully set the systemtime as above.', flush=True)
