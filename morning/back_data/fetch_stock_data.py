...
def get_day_period_data(code, from_date, until_date, db_suffix='_D'):
    ...
    db_data = list(stock_db[code + db_suffix].find({'0': {'$gte':time_converter.datetime_to_intdate(from_date), 
                                                   '$lte': time_converter.datetime_to_intdate(until_date)}}))

    days = [time_converter.intdate_to_datetime(d['0']) for d in db_data]
...
def get_day_miniute_period_data(code, from_date, until_date):
