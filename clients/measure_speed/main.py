...
def get_yesterday_amount_rank(today, db_collection):
    ...
    yesterday_list = yesterday_list[:1]
...
def get_one_min_ba_tick_quantity(ba_tick_data, current):
    from_datetime = current_datetime - timedelta(seconds=60)
    data = list(filter(lambda x: from_datetime <= x['date'] <= current_datetime, tick_data)) 
    bid_volume = 0
    ask_volume = 0
    if len(data) > 0:
        for d in data:
            ...
            ask_volume += (d['first_ask_remain'] + d['second_ask_remain'] + d['thrid_ask_remain'])
...
def save_to_graph(code, datetime_arr, price_arr, buy_speed_arr, sell_speed_arr, amount_arr):
    ...
    fig.write_html(code+'.html', auto_open=False)
