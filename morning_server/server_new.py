...
from morning_server import morning_stats_new as morning_stats
...
def handle_response(sock, header, body):
    ...
    logger.debug('HANDLE RESPONSE DONE')
...
def handle_request_cybos(sock, header, body):
    ...
    if data is None:
        ...
        logger.debug('HEADER(to collector) ' + str(header))
        ...
        logger.debug('HEADER(cached) %s', header)
...
def handle_trade_request(sock, header, body):
    ...
    if header['method'] == message.TRADE_DATA:
        ...
        handle_trade_block_request(sock, header, body)
