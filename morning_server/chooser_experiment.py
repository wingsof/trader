...
while from_date <= until_date:
    ...
    for code in kosdaq_code:
        ...
        df.append({'date': time_converter.intdate_to_datetime(from_date),
                    'code': code,
                    'profit': profit,
                    'highest_time': highest_time
        })
    yesterday_datas = sorted(yesterday_datas, lambda x: x['amount'], reverse=True)
    for i, y in enumerate(yesterday_datas):
        df[df['code'] == y['code']]['yesterday_rank'] = i + 1
    from_date += timedelta(days=1)
