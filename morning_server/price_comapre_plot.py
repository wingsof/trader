...
def save(message_reader, peak_data, data, start_time, until_time, matched_data):
    ...
    for m in matched_data:
        ...
        min_data_price_array = [int((mdata['lowest_price'] + mdata['close_price'] + mdata['highest_price'])/3) for mdata in min_data_c]
        min_data_date_array = []
        ...
        titles.append(m['code'] + '_' + m['from_date'].strftime('%Y%m%d-') + m['until_date'].strftime('%Y%m%d'))


    fig = make_subplots(rows=len(matched_data_price_array) + 2, 
                        cols=1, 
                        shared_xaxes=True, 
                        specs=[[{"secondary_y": True}]] * (len(matched_data_price_array) + 2), 
    ...
    for p in peak_data['peak'][1:]:
        annotations.append(go.layout.Annotation(x=p['time'],
                                y=p['price'],
